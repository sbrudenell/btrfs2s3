...
def test_ignore_snapshots_from_unrelated_sources(
    ...
    if impl == Impl.Assessor:
        ...
        plan = Plan.create()
        with (
            Source.create(source1) as source1_,
            SnapshotDir.create(snapshot_dir) as snapshot_dir_,
        ):
            remote = Remote.create(name="test", s3=s3, bucket=bucket)
            with plan.update() as update:
...
def test_second_run_is_a_no_op(
    ...
    if impl == Impl.Assessor:
        ...
        with (
            ...
            assert plan.created_snapshots == {}
            assert plan.rename_snapshots == []
            assert plan.upload_backups == []
            assert plan.delete_backups == []
            assert plan.destroy_snapshots == []
