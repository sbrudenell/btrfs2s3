...
def test_get_nominal_none() -> None:
    index = _Index(iter_time_spans=iter_time_spans, items=())
    got = index.get_nominal((arrow.get(0), arrow.get(0)))
    assert got is None


def test_get_nominal_snapshot_one() -> None:
    snapshot = mksubvol()
    index = _Index(iter_time_spans=iter_time_spans, items=(snapshot,))
    for timespan in iter_time_spans(snapshot.ctime):
...
def test_get_nominal_snapshot_one_of_many() -> None:
    ...
    index = _Index(iter_time_spans=iter_time_spans, items=(snapshot1, snapshot2))
    for timespan in iter_time_spans(snapshot1.ctime):
        got = index.get_nominal(timespan)
...
def test_get_nominal_backup_one() -> None:
    ...
    index = _Index(iter_time_spans=iter_time_spans, items=(backup,))
    for timespan in iter_time_spans(backup.ctime):
        got = index.get_nominal(timespan)
        assert got == backup
...
def test_get_nominal_backup_one_of_many() -> None:
    ...
    index = _Index(iter_time_spans=iter_time_spans, items=(backup1, backup2))
    for timespan in iter_time_spans(backup1.ctime):
        got = index.get_nominal(timespan)
        assert got == backup1
...
def test_get_all_time_spans() -> None:
    ...
    index = _Index(iter_time_spans=iter_time_spans, items=(snapshot,))
    got = index.get_all_time_spans()
    assert got == set(iter_time_spans(snapshot.ctime))
