...
from btrfs2s3._internal.cost import TIMEFRAMES
...
def _check_timeframe(v: Any) -> None:  # noqa: ANN401
    check_string(v)
    if v not in TIMEFRAMES:
        msg = f"Timeframe must be one of {', '.join(TIMEFRAMES)}"
        raise InvalidConfigError(msg)
...
_COST_PER_BYTES_AND_TIME_SCHEMA = Map(
    ...
    Optional("per_time", _check_timeframe, "month"),
)
...
_S3_COSTS_SCHEMA = Map(
    ...
    Optional("storage_time_granularity", _check_timeframe, "hour"),
    Optional("billing_period", _check_timeframe, "month"),
)
